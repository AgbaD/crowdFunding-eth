  ...
  return (
    <div className='flex md:flex-row flex-col-reverse justify-between mb-[35px] gap-6'>
      ...
      <div className="sm:flex hidden flex-row justify-end gap-4">
        ...
        <Link to='/profile'>
          <div className='w-[52px] h-[52px] rounded-full bg-[#2c2f31] flex justify-center items-center cursor-pointer'>
      ...
      <div className='sm:hidden flex justify-between items-center relative'> 
        <div className='w-[40px] h-[40px] rounded-[10px] bg-[#2c2f31] flex justify-center items-center cursor-pointer'>
          <img src={profile} alt='user' className='w-[60%] h-[60%] object-contain'/>
        </div>
        <img src={menu} alt="menu" className='w-[34px] h-[34px] object-contain cursor-pointer'
        onClick={() => setToggleDrawer((prev) => !prev)}/>
        <div className={`absolute top-[60px] right-0 left-0 bg-[#1c1c24] z-10 shadow-secondary py-4 ${!toggleDrawer ? '-translate-x-[100vw]' : 'translate-x-0'} transition-all duration-700`}>
          <ul className="mb-4">
            {navlinks.map((link) => (
